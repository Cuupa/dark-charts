  ...
  return <header className="site-header">
    <div className="site-header-inner">
      ...
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="sm" onClick={() => setLanguage(language === 'de' ? 'en' : 'de')} aria-label={t('a11y.switchLang')}>{language.toUpperCase()}</Button>
        ...
        <Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button className="md:hidden" variant="ghost" size="icon" aria-label={t('ui.menu')}><List size={24} /></Button></SheetTrigger><SheetContent className="bg-card"><SheetTitle>Dark Charts</SheetTitle><nav aria-label={t('a11y.nav')} className="flex flex-col gap-4 py-8">{links}<Link href="/methodology" onClick={() => setOpen(false)}>{t('footer.methodology')}</Link><Link href="/profile" onClick={() => setOpen(false)}>{t('nav.profile')}</Link></nav></SheetContent></Sheet>
      </div>
    </div>
  </header>;
}
