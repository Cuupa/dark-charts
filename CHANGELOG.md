...
### Added
- Demo admin login on `/login` (`Preview admin area`) is in-memory only: no `users` row, works with no database. Sets `dc-demo-token` for `/admin`. Off in production unless `ALLOW_DEMO_LOGIN=1`.
